...
当前版本 **0.33.0**，发布到 npm。宿主 `package.json` **精确钉** `"nfx-ui": "0.33.0"`（不要 `^0.31.0`）。
...
改 hooks（例如手机号登录）后：**升版本 → push `main`（等几分钟让 GitHub Actions 发布 npm）→ 所有宿主把 pin 改成新版本并 `npm install`**。不要让 Identity 用 0.33.0、News 用 0.29.0。
